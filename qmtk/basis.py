...
from qmtk.utils.preprocess import alias
...
class MBOp(OpBase):
    ...
    def spmat(self):
        return kron.prod(each.spmat() for each in self.ops)

    def spu(self):
        return kron.prod(each.u() for each in self.ops)

    def spinvu(self):
        return kron.prod(each.invu() for each in self.ops)
